...
const wordcloud_1 = __importDefault(require("wordcloud"));
...
class Taggy {
    ...
    createWordCloud(htmlElement) {
        return (0, wordcloud_1.default)(htmlElement, {
            list: [
                ["foo", 12],
                ["bar", 6],
            ],
    ...
    async processInput(input) {
        ...
        for (const category of this.glossaryData.tags) {
            if (this.config.include_top) {
                ...
                glossaryTags.push((0, normalize_for_search_1.default)(category.name));
            }
        ...
        if (this.config.assign_top) {
            ...
            this.glossaryData.tags.forEach((category) => {
                ...
                finalSet.forEach((element) => {
                    // if INCLUDE_TOP ist set -> add top categories
                    if ((0, normalize_for_search_1.default)(category.name) == element) {
                        count += 1;
                    }
